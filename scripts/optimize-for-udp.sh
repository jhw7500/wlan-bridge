...
if command -v ethtool &> /dev/null; then
    ...
    ethtool -G $WLAN_IF rx 4096 tx 4096 2>/dev/null && \
        echo "  ✓ $WLAN_IF: Ring buffer 조정 실패 (드라이버 미지원 가능)${NC}"
else
...
# read -p 는 인터랙티브 환경에서만 가능하므로 실제 실행 시 주의
echo "  ⓘ 재부팅 시 설정 유지를 원하시면 /etc/sysctl.conf에 위 설정들을 추가하세요."
...
echo ""
echo "1. 브릿지 시작 (wbridge-tpacket 권장):"
echo -e "   ${YELLOW}sudo wbridge-tpacket $ETH_IF $WLAN_IF${NC}"
echo ""
...
echo ""
echo "3. 브릿지 통계 확인:"
echo -e "   ${YELLOW}sudo kill -USR1 \$(pidof wbridge-tpacket)${NC}"
echo ""
echo -e "${YELLOW}주의:${NC}"
echo "  • 100-200M 대역폭 권장"
echo ""
