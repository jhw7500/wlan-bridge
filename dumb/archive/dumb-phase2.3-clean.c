...
struct ring_buffer {
    struct ring_packet packets[RING_SIZE];
    atomic_uint head __attribute__((aligned(64)));  // Producer writes here
    atomic_uint tail __attribute__((aligned(64)));  // Consumer reads here
    atomic_ulong overflow;  // Counter for ring full events
...
// Ring buffers: if0→if1 and if1→if0
static struct ring_buffer rings[2];
...
// Ring buffer 설정
#define RING_SIZE 1024  // Must be power of 2 for fast modulo
